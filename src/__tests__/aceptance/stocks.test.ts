...
it('Should stock file exists', async() => {
  let checkFileExistence = await fileExists(stockRelativePath);
...
it('Should stock file readable', async() => {
  let checkFileReadable = await fileReadable(stockRelativePath);
...
it('Should stock file json', async() => {
  const data = readFileSync( stockRelativePath , { encoding: 'utf8' } );
